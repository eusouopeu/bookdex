...
export default function EffectsSection({
  ...
  const list = Object.values(profiles || {}).sort((a, b) => b.createdAt - a.createdAt);
  const openProfile = openId ? profiles[openId] : null;
  ...
  if (openProfile) {
    return (
      <EffectProfileDetail
        ...
        onUpdateItemRating={onUpdateItemRating}
  ...
  return (
    <div>
      ...
      {list.map((profile) => {
        ...
        const activeCount = profile.items.filter((it) => it.active).length;
        return (
          <button
            key={profile.id}
              ...
              <div style={{ fontFamily: "Inter, sans-serif", fontSize: "11px", color: "var(--text-muted)" }}>
                ...
                {profile.criteria.length > 0 &&
                  " · " +
                    profile.criteria
                      .slice(0, 3)
                      .map((c) => `${c.label}: ${(totals[c.id] || 0) > 0 ? "+" : ""}${totals[c.id] || 0}`)
                      .join(" · ")}
              </div>
            </div>
